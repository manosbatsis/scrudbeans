...
</RPK></R></IRPK></IR></ILPK></IL></LPK></L> */
@MappedSuperclass
@JsonSerialize(using = EmbeddableCompositeIdSerializer::class)
@JsonDeserialize(using = EmbeddableCompositeIdDeserializer::class)
abstract class AbstractEmbeddableQuadrupleIdentifier<L, LPK : Serializable?, IL, ILPK : Serializable?, IR, IRPK : Serializable?, R, RPK : Serializable?> :
    ...
    override fun equals(obj: Any?): Boolean {
        ...
        return if (AbstractEmbeddableQuadrupleIdentifier::class.java.isAssignableFrom(obj.javaClass)) {
            val other = obj as AbstractEmbeddableQuadrupleIdentifier<*, *, *, *, *, *, *, *>
            EqualsBuilder()
    ...
    override fun init(value: @NotNull String?) {
        val parts = value!!.split(SPLIT_CHAR.toRegex()).dropLastWhile { it.isEmpty() }
            .toTypedArray()
        if (parts.size == 4 && StringUtils.isNoneBlank(parts[0], parts[1], parts[2], parts[3])) {
            left = buildLeft(parts[0])
            innerLeft = buildInnerLeft(parts[1])
            innerRight = buildInnerRight(parts[2])
            right = buildRight(parts[3])
        } else {
            throw IllegalArgumentException("Given value must have four non-blank parts separated by '_'.")
        }
    }
